...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    if (op==A_ADD) {
        long_t tmp = (argA < 0 == argB < 0) && (compute_alu(op,argA,argB) < 0 != argA < 0);
        ovf = tmp? TRUE:FALSE;
    }
    else if (op==A_SUB) {
        if (argA > 0 && argB < 0 && val > 0)
            ovf = TRUE;
        else if (argA < 0 && argB > 0 && val < 0)
            ovf = TRUE;
...
bool_t cond_doit(cc_t cc, cond_t cond) 
{
    ...
    switch(cond) {
        case 0:
            ...
            break;
        case 1:
            if(GET_SF(cc)^GET_OF(cc) | GET_ZF(cc))
                doit = TRUE;
            break;
        case 2:
...
stat_t nexti(y86sim_t *sim)
{
    ...
    switch(icode) {
        case 3:case 4:case 5:case 7:case 8:
            if(!get_long_val(sim->m, next_pc, &i_immediate)) {
                err_print("PC = 0x5x, Invalid instruction address", sim->pc)
                return STAT_ADR;
            }
            next_pc += 4;
            break;
    ...
    switch (icode) {
      ...
      case I_RRMOVL:  /* 2:x regA:regB */        
        if (cond_doit(sim->cc, ifun)) {
            regAval =  get_reg_val(sim->r,iregA);
            set_reg_val(sim->r, iregB, regAval);
        }
        sim->pc = next_pc;
        break;
         
      case I_IRMOVL:  /* 3:0 F:regB imm */       
        set_reg_val(sim->r, iregB, i_immediate);
        sim->pc = next_pc;
        break;

      case I_RMMOVL: /* 4:0 regA:regB imm */        
        regAval = get_reg_val(sim->r,iregA);
        regBval = get_reg_val(sim->r,iregB);
        set_long_val(sim->m,i_immediate+regBval,regAval);
        sim->pc = next_pc;
        break;

      case I_MRMOVL: /* 5:0 regB:regA imm */
        if(ifun!=0 ||!NORM_REG(iregA) || !NORM_REG(iregB))
            return STAT_INS;
        regBval = get_reg_val(sim->r,iregB);
        if (!get_long_val(sim->m,i_immediate+regBval,&tmp)) {
        err_print("PC = 0x%x, Invalid data address 0x%x", sim->pc, i_immediate+regBval);
            return STAT_ADR;
        }
        set_reg_val(sim->r,iregA,tmp);
        sim->pc = next_pc;
        break;
      ...
      case I_ALU: /* 6:x regA:regB */
        regAval = get_reg_val(sim->r, iregA);
        regBval = get_reg_val(sim->r, iregB);
        tmp = compute_alu(ifun, regAval, regBval);
        set_reg_val(sim->r, iregB, tmp);
        sim->pc = next_pc;
        sim->cc = compute_cc(ifun, regAval, regBval, tmp);
        break;

      case I_JMP: /* 7:x imm */
        if (cond_doit(sim->cc, ifun)) {
            if (!get_byte_val(sim->m, i_immediate, &codefun)) {
                err_print("PC = 0x5x, Invalid instruction address", i_immediate)
                return STAT_ADR;
            }
            sim->pc = i_immediate;     
        }
      ...
      case I_CALL: /* 8:x imm */       
        regAval = get_reg_val(sim->r, REG_ESP);
        set_reg_val(sim->r, REG_ESP, regAval-4);
        if (!set_long_val(sim->m, regAval-4, next_pc)) {
            err_print("PC = 0x%x, Invalid stack address 0x%x",sim->pc,regAval-4);
            return STAT_ADR;
        }
        set_long_val(sim->m, regAval-4, next_pc);
        sim->pc = i_immediate;
        break;

      case I_RET: /* 9:0 */
        regAval = get_reg_val(sim->r,REG_ESP);
        if (!get_long_val(sim->m, regAval, &tmp)) {
            err_print("PC = 0x%x, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
        sim->pc = tmp;
      ...
      case I_PUSHL: /* A:0 regA:F */       
        regAval = get_reg_val(sim->r, iregA);
        tmp = get_reg_val(sim->r, REG_ESP);
        set_reg_val(sim->r, REG_ESP, tmp-4);

        if (!set_long_val(sim->m,tmp-4,regAval)) {
                err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc,tmp-4);
                return STAT_ADR;
        }
        sim->pc = next_pc;
        break;
      ...
      case I_POPL: /* B:0 regA:F */
        regBval = get_reg_val(sim->r, REG_ESP);
        set_reg_val(sim->r, REG_ESP, regBval+4);

        if (!get_long_val(sim->m, regBval, &tmp)) {
            err_print("PC = 0x%x, Invalid data address", regBval);
            return STAT_ADR;
        }
        set_reg_val(sim->r, iregA, tmp);
        sim->pc = next_pc;
